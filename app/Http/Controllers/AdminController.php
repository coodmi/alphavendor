...
class AdminController extends Controller
{
    ...
    public function updateOrderStatus(Request $request, Order $order)
    {
        ...
        if ($order->isWholesaleOrImport()) {
            $request->validate([
                'status' => 'required|in:pending_advance_payment,advance_paid,order_confirmed,ready_for_bangladesh_delivery,pending,processing,shipped,delivered,cancelled,refunded,exchange,returned',
            ]);
        ...
        // ── Retail orders → state machine ────────────────────────────────────
        $request->validate([
            'status' => 'required|in:pending,order_confirmed,processing,shipped,delivered,cancelled',
        ]);
        ...
        \App\Services\NotificationService::orderStatusChanged($order->fresh()->load('user'));
