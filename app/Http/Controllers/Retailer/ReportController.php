...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $deliveredOrdersQuery = (clone $ordersQuery)->whereIn('status', self::DELIVERED_STATUSES);
        ...
        $productSalesGross = DB::table('order_items')
        ...
        $returnedQuantity = $this->completedReturnsQuery($vendorId, $start, $end)->sum('quantity');

        $productSales = max(0, (int) $productSalesGross - (int) $returnedQuantity);
        ...
        $returnsQuery = ReturnRequest::where('vendor_id', $vendorId)
            ->whereBetween('created_at', [$start, $end]);
        ...
        $completedReturns = (clone $returnsQuery)
        ...
        $topProducts = DB::table('order_items')
            ...
            ->select(
                'products.id',
                'products.name',
                DB::raw('SUM(order_items.quantity) as gross_sold')
            )
