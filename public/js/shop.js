...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const vendorTypeCheckboxes = document.querySelectorAll('.vendor-type-checkbox');
    const minPriceInput = document.querySelector('.price-input input[type="number"]:first-child');
    const maxPriceInput = document.querySelector('.price-input input[type="number"]:last-child');
    ...
    if (brandSearch) {
        brandSearch.addEventListener('input', function() {
            ...
            const brandItems = document.querySelectorAll('.filter-box:has(.search-filter) .filter-list li');
    ...
    function applyFilters() {
        ...
        const selectedVendorTypes = Array.from(document.querySelectorAll('.vendor-type-checkbox:checked'))
            .map(cb => cb.value);
        selectedVendorTypes.forEach(type => params.append('vendor_types[]', type));
        ...
        // Get price range
        if (minPriceInput && minPriceInput.value && minPriceInput.value !== '0') {
            params.append('min_price', minPriceInput.value);
        }
        if (maxPriceInput && maxPriceInput.value && maxPriceInput.value !== '1000') {
    ...
    function initializeFiltersFromURL() {
        ...
        vendorTypes.forEach(type => {
            const checkbox = document.querySelector(`.vendor-type-checkbox[value="${type}"]`);
            if (checkbox) checkbox.checked = true;
        });
        ...
        if (rating) {
            const checkbox = document.querySelector(`.rating-checkbox[value="${rating}"]`);
            if (checkbox) checkbox.checked = true;
        }
