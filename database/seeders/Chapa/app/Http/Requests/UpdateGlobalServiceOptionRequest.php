...
class UpdateGlobalServiceOptionRequest extends FormRequest
{
    /**
    ...
    {
        $optionId = $this->route('global_service_option') 
            ? $this->route('global_service_option')->id 
            : $this->route('option');

        return [
            'name_en' => ['required', 'string', 'max:255', Rule::unique('global_service_options', 'name_en')->ignore($optionId)],
