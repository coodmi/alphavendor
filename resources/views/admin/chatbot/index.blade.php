...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex gap-1 mb-6 border-b border-gray-200">
        ...
        <button onclick="switchTab('faqs')" id="tab-faqs"
            class="chat-tab px-5 py-2.5 text-sm font-semibold border-b-2 border-transparent text-gray-200 hover:text-white -mb-px">
            ❓ Quick Replies / FAQ
        </button>
    ...
    <div id="panel-inbox">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6" style="height:600px;">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-y-auto">
                <div class="px-4 py-3 border-b border-gray-100 font-semibold text-white text-sm">
                ...
                <div onclick="loadConversation({{ $conv->id }})"
                    ...
                    <p class="text-xs text-gray-200 truncate">{{ $conv->latestMessage?->message ?? 'No messages' }}</p>
                    <span class="inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium
                        ...
                        {{ $conv->status === 'closed' ? 'bg-gray-100 text-gray-200' : '' }}">
            ...
            <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
                ...
                <div id="chat-panel" class="hidden flex-col h-full">
                    <div class="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
                        <div>
                            <p class="font-semibold text-white" id="panel-user-name">—</p>
    ...
    <div id="panel-faqs" class="hidden">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="font-bold text-white mb-4">Add Quick Reply / FAQ</h3>
                <form action="{{ route('admin.chatbot.faqs.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-100 mb-1">Question / Keyword</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-100 mb-1">Answer</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-100 mb-1">Sort Order</label>
            ...
            {{-- FAQ List --}}
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="font-bold text-white mb-4">Existing FAQs ({{ $faqs->count() }})</h3>
                @forelse($faqs as $faq)
                <div class="border border-gray-100 rounded-lg p-4 mb-3">
                    <div class="flex justify-between items-start gap-3">
                        <div class="flex-1">
                            <p class="font-semibold text-sm text-white">{{ $faq->question }}</p>
                            <p class="text-xs text-gray-200 mt-1 line-clamp-2">{{ $faq->answer }}</p>
                        </div>
    ...
    <div id="panel-settings" class="hidden">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-xl">
            <h3 class="font-bold text-white mb-5">Chat Widget Settings</h3>
            <form action="{{ route('admin.chatbot.update') }}" method="POST" class="space-y-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <label for="chatbot_enabled" class="text-sm font-medium text-white">Enable Chat Widget</label>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-100 mb-1">Chat Name</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-100 mb-1">Welcome Message</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-100 mb-1">Input Placeholder</label>
                    <input type="text" name="chatbot_placeholder" value="{{ $settings['chatbot_placeholder'] ?? 'Type your message...' }}"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-400">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-100 mb-1">Theme Color</label>
...
<div id="editFaqModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
        <h3 class="font-bold text-white mb-4">Edit FAQ</h3>
        <form id="editFaqForm" method="POST" class="space-y-4">
            @csrf @method('PUT')
            <div>
                <label class="block text-sm font-medium text-gray-100 mb-1">Question</label>
            ...
            <div class="flex gap-3 justify-end">
                <button type="button" onclick="document.getElementById('editFaqModal').classList.add('hidden')"
                    class="px-4 py-2 bg-gray-100 text-white rounded-lg text-sm font-medium">Cancel</button>
