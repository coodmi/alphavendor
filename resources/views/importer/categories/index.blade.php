...
<div class="mb-8">
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-3xl font-bold text-gray-800 mb-1">Categories Management</h2>
            <p class="text-gray-600">Manage import product categories with SEO meta fields</p>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Meta Title</th>
                ...
                <tr id="category-row-{{ $category->id }}" class="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4">
                        ...
                        @if($category->meta_keywords)
                            <div class="text-xs text-gray-500 mt-1">{{ Str::limit($category->meta_keywords, 60) }}</div>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                        @if($category->parent)
    ...
    <div class="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <form id="categoryForm" class="p-6 space-y-5">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Link your import category to an admin category</p>
            </div>
            ...
            <div class="border border-gray-200 rounded-xl p-4 bg-gray-50 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="meta_title" id="categoryMetaTitle" maxlength="255"
                           placeholder="e.g., Import Electronics - Wholesale Supplier"
                ...
                <div>
                    ...
                    <textarea name="meta_description" id="categoryMetaDescription" rows="3" maxlength="500"
...
function editCategory(category) {
    ...
    document.querySelector('input[name="is_active"]').value = category.is_active ? '1' : '0';
    validateCatKeywords(document.getElementById('categoryMetaKeywords'));

    if (category.image) {
        const isUrl = category.image.startsWith('http');
        document.getElementById('previewImg').src = isUrl ? category.image : `/storage/${category.image}`;
...
document.getElementById('categoryForm').addEventListener('submit', function(e) {
    ...
    const keywords = document.getElementById('categoryMetaKeywords').value;
    const kws = keywords.split(',').map(k => k.trim()).filter(k => k.length > 0);
    if (kws.length < 5) {
        showToast('Please provide at least 5 meta keywords separated by commas.', 'error');
        return;
    }
...
window.onclick = function(event) {
    if (event.target.id === 'categoryModal') closeModal();
