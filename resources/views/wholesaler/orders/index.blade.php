    ...
    <div class="bg-white rounded-lg shadow-md">
        <div class="p-6 border-b border-gray-200">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ route('wholesaler.dashboard') }}" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 transition">
                    Back to Dashboard
                </a>
            </div>
        </div>
                        ...
                        @foreach($orders as $order)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <a href="{{ route('wholesaler.orders.show', $order) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                    @if(in_array($order->status, ['order_confirmed', 'processing']))
                                    <form action="{{ route('wholesaler.orders.update-status', $order) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('PATCH')
