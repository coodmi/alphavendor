...
<div class="mb-8">
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-3xl font-bold text-white mb-1">Categories Management</h2>
            <p class="text-gray-100">Manage product categories</p>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Category Name</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Parent Category</th>
            ...
            <tbody id="categoriesTableBody" class="bg-white divide-y divide-gray-200">
                ...
                <tr id="category-row-{{ $category->id }}" class="hover:bg-gray-50 transition-colors duration-150">
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($category->image)
                            @if(filter_var($category->image, FILTER_VALIDATE_URL))
                                <img src="{{ $category->image }}" alt="{{ $category->name }}" class="w-14 h-14 object-cover rounded-lg shadow-sm">
                            @else
                                <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-14 h-14 object-cover rounded-lg shadow-sm">
                            @endif
                        @else
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div class="text-sm text-gray-200">{{ $category->description ? Str::limit($category->description, 50) : 'No description' }}</div>
                    </td>
                    <td class="px-6 py-4">
                        @if($category->parent)
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-white">{{ $category->sort_order }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <button data-category='@json($category)' onclick="editCategoryFromData(this)" class="inline-flex items-center px-3 py-2 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-colors duration-150 mr-2">
                            <i class="fas fa-edit mr-1"></i> Edit
                        </button>
                        <button onclick="confirmDelete({{ $category->id }}, '{{ addslashes($category->name) }}')" class="inline-flex items-center px-3 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors duration-150">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-16 text-center">
                        ...
                        <p class="text-gray-200 text-lg">No categories found. Click "Add Category" to create one.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
...
<div id="categoryModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-center bg-gradient-to-r from-indigo-600 to-purple-600">
            <h3 class="text-xl font-bold text-white" id="modalTitle">Add Category</h3>
        ...
        <form id="categoryForm" class="p-6 space-y-5">
            ...
            <div>
                <label class="block text-sm font-semibold text-white mb-2">Parent Category (Admin) *</label>
                ...
                <p class="text-xs text-gray-200 mt-1">Link your custom category to an admin category</p>
            </div>

            <div>
                <label class="block text-sm font-semibold text-white mb-2">Category Name *</label>
                <input type="text" name="name" id="categoryName" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200">
            </div>

            <div>
                <label class="block text-sm font-semibold text-white mb-2">Description</label>
                <textarea name="description" id="categoryDescription" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200"></textarea>
            </div>

            <div>
                <label class="block text-sm font-semibold text-white mb-2">Category Image</label>
                ...
                <div id="uploadSection">
                    <input type="file" name="image" id="categoryImage" accept="image/*" onchange="previewImageFile(event)" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                </div>

                <!-- URL Input -->
                <div id="urlSection" class="hidden">
                    <input type="url" name="image_url" id="categoryImageUrl" placeholder="https://example.com/image.jpg" onchange="previewImageUrl()" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                </div>
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="categorySortOrder" value="0" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200">
            </div>

            <div>
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" id="categoryStatus" value="1" checked class="w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                    <span class="ml-3 text-sm font-medium text-white">Active Category</span>
                </label>
            </div>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl max-w-md w-full shadow-2xl transform transition-all duration-300">
        <div class="p-6">
            ...
            <h3 class="text-2xl font-bold text-white text-center mb-2" id="deleteModalTitle">Delete Category?</h3>
...
function editCategory(category) {
    ...
    if (category.image) {
        ...
        const imgSrc = isUrl ? category.image : `/storage/${category.image}`;
...
function executeSoftDelete() {
    ...
    fetch(`/wholesaler/categories/${deleteTargetId}`, {
        method: 'DELETE',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            document.getElementById(`category-row-${deleteTargetId}`).remove();
...
document.getElementById('categoryForm').addEventListener('submit', function(e) {
    ...
    const url = editingCategoryId ? `/wholesaler/categories/${editingCategoryId}` : '{{ route('wholesaler.categories.store') }}';
